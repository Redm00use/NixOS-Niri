...
from .common import LOG_FILE, print_progress
...
def main() -> int:
    ...
    filesystem = args.fs or imported.get("fs") or ("ext4" if preset == "vm" else choose_filesystem())
    disk = args.disk or imported.get("disk") or prompt_disk()
    separate_home = args.separate_home or imported.get("separate_home") or (False if preset == "vm" else choose_yes_no("Отдельный раздел /home?", False))
    ...
    swap_size_gib = args.swap_size_gib if args.swap_size_gib is not None else imported.get("swap_size_gib") or (4 if preset == "vm" else prompt_int("Размер swap в GiB (0 = без swap-раздела)", 8))
    luks_enabled = args.luks or imported.get("luks") or (False if preset == "vm" else choose_yes_no("Включить LUKS для root?", False))
    ...
    try:
        ...
        print("\nОшибка установки.")
        print(f"Стадия: Установка / финализация")
